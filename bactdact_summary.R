...
for(i in GPSCs){
    ...
    #no dates iter load
    filename <- paste("GPSC", i, "_bactdate.results_nodates.RData", sep = "")
    if(file.exists(filename)){
    tmp <- load(file = paste("GPSC", i, "_bactdate.results_nodates.RData", sep = ""))
    results_no_dates <- get(tmp)
    } else {
    results_no_dates <- "NA"
    }
    ...
    gpsc_out <- matrix(nrow = 1, ncol = 12)
    ...
    if(results_no_dates=="NA"){
      model_test1 <- "NA;NA;NA"
    } else {
      model_test1 <- modelcompare_out(results_r1,results_no_dates)
    }
    if(results_rand_dates=="NA"){
      model_test2 <- "NA;NA;NA"
    } else {
      model_test2 <- modelcompare_out(results_r1,results_rand_dates)
    }
    gpsc_out[1,10] <- as.character(sprintf('%s; %s; %s',model_test1[1],model_test1[2],model_test1[3]))
    gpsc_out[1,11] <- as.character(sprintf('%s; %s; %s',model_test2[1],model_test2[2],model_test2[3]))
    converg_test <-gelman.diag(coda_iters, confidence = 0.95, transform=FALSE, autoburnin=FALSE,multivariate=TRUE)
    gpsc_out[1,12] <- as.character(sprintf('%.2f; %.2f; %.2f',converg_test$psrf[4],converg_test$psrf[5],converg_test$psrf[6]))
    out <- rbind(out, gpsc_out)

}

colnames(out) <- c('GPSC','rootprob','likelihood','prior','mu','sigma','alpha','Root date', 'ESS(mu; sigma; alpha)', 'DIC(dates;nodates;verdict)','DIC(dates;random;verdict)', 'gelman(mu;sigma,alpha)')

#write.csv(out,file="bactdate.replicates_burn10_results.csv",row.names=FALSE)
write.csv(out,file="GPSC2_bactdate.replicates_burn10_results.csv",row.names=FALSE)
